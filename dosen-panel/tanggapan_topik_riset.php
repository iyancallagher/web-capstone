...
<body>
    ...
    <div class="container mt-5 p-4" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
        ...
        <form method="POST">
            <div class="mb-4">
                ...
                <textarea name="tanggapan_topik_riset" id="tanggapan" rows="4" class="form-control" required><?php echo htmlspecialchars($data['tanggapan_topik_riset'], ENT_QUOTES, 'UTF-8'); ?></textarea>
            </div>
            <div class="mb-4">
    ...
    if (isset($_POST['simpan'])) {
        $tanggapan = $_POST['tanggapan_topik_riset']; // Ambil tanggapan dosen
        $status = $_POST['status']; // Ambil status baru
        
        // Query untuk menyimpan tanggapan dan status
        $querySimpan = mysqli_query($con, "UPDATE pendaftar_mahasiswa SET tanggapan_topik_riset='$tanggapan', status='$status' WHERE nim='$data[nim]'");
